...
## 音频
### 节点挂AudioSource组件
在节点上挂AudioSource组件，然后添加clip，开启playOnAwake或者在代码中获取到该AudioSource组件之后播放

**优点：**
* 支持可视化操作，通过拖拽绑定
* 音频和节点直接绑定，对应关系清晰
* 每个节点管理自己的音频状态，且节点销毁时音频自动停止

**缺点：**
* 不太适用于跨场景音频
* 不好进行音量之类的统一管理，额外写个配置类的话就不内聚
* 只支持懒加载，无法实现动态加载

适合只属于特定场景的背景音乐或者不在多处使用的音效

### 使用全局AudioMgr播放
在`AudioMgr`中创建持久化节点，添加`AudioSource`，封装好播放音效的函数，任何需要播放音效的位置都只要传入`clip`或url就可以播放。需要注意的是，背景音乐和音效我觉得分开成两个`AudioSource`比较好，这样可以分开控制且能同时播放

**优点:**
* 是作为工具类写的，可复用性强，换个项目可以直接拿来使用
* 在代码中可以很方便的进行音量等的统一调控
* 控制灵活，支持复杂逻辑
* 可以选择懒加载还是动态加载
* 支持跨场景播放
* 内聚性强，音频播放、配置等都在同一个类内
